...
ただ、カスタムユーザーモデルの実装は容易ではない。カスタムユーザーモデル用のアプリを作り、Djangoのデフォルトのユーザーモデルを継承し、マイグレーションをする。
...
1. カスタムユーザーモデルを動かすための専用のアプリを作る(startapp)
...
## カスタムユーザーモデルを動かすための専用のアプリを作る(startapp)

続いて、カスタムユーザーモデルを動かすためのアプリを作る。今回は`users`アプリを作る。
...
カスタムユーザーモデルを実装させることで、会員登録時に性別や年齢などの基本情報の入力を必須とさせることができる。会員登録時に指定させることもできなくはないが、かえってユーザビリティを損ねたり、開発工数が増える可能性もある。

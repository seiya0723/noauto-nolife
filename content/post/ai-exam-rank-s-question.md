...
## Q: Transformerの層はどのようになっているのか？

A. 以下の通り、Self-Attention層で作った出力ベクトルを、入力値と残差接続(足し算)をする。

その上でLayer Normalizationで平均を0分散を1に正規化する。
...
Attention機構の仕組みは、Q,K,Vを使った重み付けとベクトルの合成を行っている。(※ QとKの内積に対してKのチャンネル数の平方根で割り、Softmax関数に掛けた後、Vを掛け算する。そうして重み付けが出来上がる。)

このTransformerではAttentionで出力されたベクトルと、入力値を残差接続(足し算)している。

この残差接続では、バッチ内にある可変長の系列データの足し算になる。可変長では平均と分散を取ることが難しい。そこで、Layer Normalizationを使う。

ちなみに同じ残差接続をしているResNetは常に固定長のデータ。Transformerと違い層単位での正規化をする必要はない。バッチ単位の正規化であるBatch Normalization は使用されているが、それはResNetに限らず他CNNモデルでも使われている。

...
DBの設定関係をスキップするため、SQLiteを使用する。プロジェクト内の`database`ディレクトリにdb.sqliteを作る
...
続いて、デフォルトではMySQLを使っている状況になっている設定をSQLiteであるdb.sqliteを使用するように修正する。プロジェクトディレクトリの中にある`.env`ファイルを編集する。
    ...
    DB_DATABASE=/home/akagi/Documents/programming/php/laravel_test01/test_bbs/database/db.sqlite

既に書き込まれているDB設定は#でコメントアウトする。DB_DATABASEは上記のように絶対パスで指定しないと動かない点に注意。そのためDB_DATABASEのパスは環境に応じて適宜変更する。
...
上記コマンドで`app/Http/Controllers/`にPagesController.phpが作られる。これを編集する。
    ...
    class PagesController extends Controller
    {
        ...
        public function save(Request $request) {
            ...
            //topicのnameに受け取ったnameを入れる。ただし存在する場合に限る
            if ($request->name) $topic->name = $request->name;
